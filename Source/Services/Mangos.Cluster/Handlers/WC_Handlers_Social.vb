...
#Region "Framework"
        ...
        Public Sub SendFriendList(ByRef client As WC_Network.ClientClass, ByRef character As WcHandlerCharacter.CharacterObject)
            ...
            Dim smsgFriendList As New Packets.PacketClass(OPCODES.SMSG_FRIEND_LIST)
            If q.Rows.Count > 0 Then
                ...
                For Each r As DataRow In q.Rows
                    ...
                    If _WorldCluster.CHARACTERs.ContainsKey(guid) AndAlso _WorldCluster.CHARACTERs(guid).IsInWorld Then
                        If _WorldCluster.CHARACTERs(guid).DND Then
                            smsgFriendList.AddInt8(FriendStatus.FRIEND_STATUS_DND)
                        ElseIf _WorldCluster.CHARACTERs(guid).AFK Then
                            smsgFriendList.AddInt8(FriendStatus.FRIEND_STATUS_AFK)
                        Else
                            smsgFriendList.AddInt8(FriendStatus.FRIEND_STATUS_ONLINE)
                        End If
                        smsgFriendList.AddInt32(_WorldCluster.CHARACTERs(guid).Zone)    'Area
                        smsgFriendList.AddInt32(_WorldCluster.CHARACTERs(guid).Level)   'Level
                        smsgFriendList.AddInt32(_WorldCluster.CHARACTERs(guid).Classe)  'Class
                    Else
        ...
        Public Sub SendIgnoreList(ByRef client As WC_Network.ClientClass, ByRef character As WcHandlerCharacter.CharacterObject)
            ...
            'DONE: Make the packet
            Dim smsgIgnoreList As New Packets.PacketClass(OPCODES.SMSG_IGNORE_LIST)
            If q.Rows.Count > 0 Then
        ...
        Public Sub NotifyFriendStatus(ByRef objCharacter As WcHandlerCharacter.CharacterObject, s As FriendStatus)
            ...
            Dim friendpacket As New Packets.PacketClass(OPCODES.SMSG_FRIEND_STATUS)
            friendpacket.AddInt8(s)
            friendpacket.AddUInt64(objCharacter.Guid)
            For Each r As DataRow In q.Rows
                Dim guid As ULong = r.Item("guid")
                If _WorldCluster.CHARACTERs.ContainsKey(guid) AndAlso _WorldCluster.CHARACTERs(guid).Client IsNot Nothing Then
                    _WorldCluster.CHARACTERs(guid).Client.SendMultiplyPackets(friendpacket)
                End If
            Next
...
#Region "Handlers"
        ...
        Public Sub On_CMSG_WHO(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            packet.GetInt16()
            Dim levelMinimum As UInteger = packet.GetUInt32()       '0
            Dim levelMaximum As UInteger = packet.GetUInt32()       '100
            Dim namePlayer As String = _Functions.EscapeString(packet.GetString())
            Dim nameGuild As String = _Functions.EscapeString(packet.GetString())
            ...
            For i As Integer = 1 To stringsCount
                strings.Add(_CommonFunctions.UppercaseFirstLetter(_Functions.EscapeString(packet.GetString())))
            Next

            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_WHO [P:'{2}' G:'{3}' L:{4}-{5} C:{6:X} R:{7:X}]", client.IP, client.Port, namePlayer, nameGuild, levelMinimum, levelMaximum, maskClass, maskRace)
            ...
            For Each objCharacter As KeyValuePair(Of ULong, WcHandlerCharacter.CharacterObject) In _WorldCluster.CHARACTERs
                ...
                If stringsCount > 0 Then
                    ...
                    For Each stringValue As String In strings
                        ...
                        If _CommonFunctions.UppercaseFirstLetter(_Functions.GetRaceName(objCharacter.Value.Race)) = stringValue Then Continue For
                        If _CommonFunctions.UppercaseFirstLetter(_Functions.GetClassName(objCharacter.Value.Classe)) = stringValue Then Continue For
            ...
            Dim response As New Packets.PacketClass(OPCODES.SMSG_WHO)
            response.AddInt32(results.Count)
            response.AddInt32(results.Count)

            For Each guid As ULong In results
                response.AddString(_WorldCluster.CHARACTERs(guid).Name)           'Name
                If _WorldCluster.CHARACTERs(guid).Guild IsNot Nothing Then
                    response.AddString(_WorldCluster.CHARACTERs(guid).Guild.Name) 'Guild Name
                Else
                    response.AddString("")                          'Guild Name
                End If
                response.AddInt32(_WorldCluster.CHARACTERs(guid).Level)           'Level
                response.AddInt32(_WorldCluster.CHARACTERs(guid).Classe)          'Class
                response.AddInt32(_WorldCluster.CHARACTERs(guid).Race)            'Race
                response.AddInt32(_WorldCluster.CHARACTERs(guid).Zone)            'Zone ID
            Next
        ...
        Public Sub On_CMSG_ADD_FRIEND(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            If (packet.Data.Length - 1) < 6 Then Exit Sub
            packet.GetInt16()

            Dim response As New Packets.PacketClass(OPCODES.SMSG_FRIEND_STATUS)
            Dim name As String = packet.GetString()
            Dim guid As ULong = 0
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ADD_FRIEND [{2}]", client.IP, client.Port, name)

            'DONE: Get GUID from DB
            Dim q As New DataTable
            _WorldCluster.CharacterDatabase.Query(String.Format("SELECT char_guid, char_race FROM characters WHERE char_name = ""{0}"";", name), q)

            If q.Rows.Count > 0 Then
                guid = CType(q.Rows(0).Item("char_guid"), Long)
                Dim FriendSide As Boolean = _Functions.GetCharacterSide(q.Rows(0).Item("char_race"))

                q.Clear()
                _WorldCluster.CharacterDatabase.Query(String.Format("SELECT flags FROM character_social WHERE flags = {0}", CType(SocialFlag.SOCIAL_FLAG_FRIEND, Byte)), q)
                Dim NumberOfFriends As Integer = q.Rows.Count
                q.Clear()
                _WorldCluster.CharacterDatabase.Query(String.Format("SELECT flags FROM character_social WHERE guid = {0} AND friend = {1} AND flags = {2};", client.Character.Guid, guid, CType(SocialFlag.SOCIAL_FLAG_FRIEND, Byte)), q)
                ...
                ElseIf NumberOfFriends >= SocialList.MAX_FRIENDS_ON_LIST Then
                    ...
                    response.AddUInt64(guid)
                ElseIf _Functions.GetCharacterSide(client.Character.Race) <> FriendSide Then
                    response.AddInt8(FriendResult.FRIEND_ENEMY)
                    response.AddUInt64(guid)
                ElseIf _WorldCluster.CHARACTERs.ContainsKey(guid) Then
                    ...
                    End If
                    response.AddInt32(_WorldCluster.CHARACTERs(guid).Zone)
                    response.AddInt32(_WorldCluster.CHARACTERs(guid).Level)
                    response.AddInt32(_WorldCluster.CHARACTERs(guid).Classe)
                    _WorldCluster.CharacterDatabase.Update(String.Format("INSERT INTO character_social (guid, friend, flags) VALUES ({0}, {1}, {2});", client.Character.Guid, guid, CType(SocialFlag.SOCIAL_FLAG_FRIEND, Byte)))
                Else
            ...
            response.Dispose()
            q.Dispose()
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_FRIEND_STATUS", client.IP, client.Port)
        End Sub

        Public Sub On_CMSG_ADD_IGNORE(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            If (packet.Data.Length - 1) < 6 Then Exit Sub
            packet.GetInt16()
            Dim response As New Packets.PacketClass(OPCODES.SMSG_FRIEND_STATUS)
            Dim name As String = packet.GetString()
            Dim GUID As ULong = 0
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ADD_IGNORE [{2}]", client.IP, client.Port, name)

            'DONE: Get GUID from DB
            Dim q As New DataTable
            _WorldCluster.CharacterDatabase.Query(String.Format("SELECT char_guid FROM characters WHERE char_name = ""{0}"";", name), q)

            If q.Rows.Count > 0 Then
                GUID = CType(q.Rows(0).Item("char_guid"), Long)
                q.Clear()
                _WorldCluster.CharacterDatabase.Query(String.Format("SELECT flags FROM character_social WHERE flags = {0}", CType(SocialFlag.SOCIAL_FLAG_IGNORED, Byte)), q)
                Dim NumberOfFriends As Integer = q.Rows.Count
                q.Clear()
                _WorldCluster.CharacterDatabase.Query(String.Format("SELECT * FROM character_social WHERE guid = {0} AND friend = {1} AND flags = {2};", client.Character.Guid, GUID, CType(SocialFlag.SOCIAL_FLAG_IGNORED, Byte)), q)
                ...
                Else
                    ...
                    _WorldCluster.CharacterDatabase.Update(String.Format("INSERT INTO character_social (guid, friend, flags) VALUES ({0}, {1}, {2});", client.Character.Guid, GUID, CType(SocialFlag.SOCIAL_FLAG_IGNORED, Byte)))
        ...
        Public Sub On_CMSG_DEL_FRIEND(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_DEL_FRIEND", client.IP, client.Port)
            ...
            Try
                Dim q As New DataTable
                _WorldCluster.CharacterDatabase.Query(String.Format("SELECT flags FROM character_social WHERE guid = {0} AND friend = {1};", client.Character.Guid, GUID), q)
                ...
                If q.Rows.Count > 0 Then
                    ...
                    If (newFlags And (SocialFlag.SOCIAL_FLAG_FRIEND Or SocialFlag.SOCIAL_FLAG_IGNORED)) = 0 Then
                        _WorldCluster.CharacterDatabase.Update(String.Format("DELETE FROM character_social WHERE friend = {1} AND guid = {0};", client.Character.Guid, GUID))
                    Else
                        _WorldCluster.CharacterDatabase.Update(String.Format("UPDATE character_social SET flags = {2} WHERE friend = {1} AND guid = {0};", client.Character.Guid, GUID, newFlags))
                    End If
        ...
        Public Sub On_CMSG_DEL_IGNORE(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_DEL_IGNORE", client.IP, client.Port)
            If (packet.Data.Length - 1) < 13 Then Exit Sub
            packet.GetInt16()
            Dim response As New Packets.PacketClass(OPCODES.SMSG_FRIEND_STATUS)
            Dim GUID As ULong = packet.GetUInt64()
        ...
        Public Sub On_CMSG_FRIEND_LIST(ByRef packet As Packets.PacketClass, ByRef client As WC_Network.ClientClass)
            _WorldCluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_FRIEND_LIST", client.IP, client.Port)
